...
class Pacman {
    ...
    draw(imageDataA) {
        ...
        let facingAngle = Math.atan2(
            this.lastValidDirection.y,
            this.lastValidDirection.x
        );
    ...
    update() {
        ...
        const isCenteredY =
            Math.abs(this.y - (this.j * this.w + this.w / 2)) < this.speed;
        ...
        if (isCenteredX && isCenteredY) {
            ...
            let canTurn = false;
            if (this.nextDirection.x === 1 && !currentCell.walls[1])
                canTurn = true;
            else if (this.nextDirection.x === -1 && !currentCell.walls[3])
                canTurn = true;
            else if (this.nextDirection.y === -1 && !currentCell.walls[0])
                canTurn = true;
            else if (this.nextDirection.y === 1 && !currentCell.walls[2])
                canTurn = true;
            ...
            if (this.direction.x === 1 && currentCell.walls[1])
                this.direction = { x: 0, y: 0 };
            else if (this.direction.x === -1 && currentCell.walls[3])
                this.direction = { x: 0, y: 0 };
            else if (this.direction.y === -1 && currentCell.walls[0])
                this.direction = { x: 0, y: 0 };
            else if (this.direction.y === 1 && currentCell.walls[2])
                this.direction = { x: 0, y: 0 };
        }
